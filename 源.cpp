...
ofstream country_colors_output ( ".\\common\\country_colors.txt" , ios::app );	//country_colors.txt输出
ofstream issues_output ( ".\\common\\issues.txt" , ios::app );	//isseus.txt输出
...
ifstream issues_input ( ".\\common\\issues.txt" );	//isseus.txt输入
...
vector<economic_policy> economic_policy_vector;	//储存经济政策
vector<citizenship_policy> citizenship_policy_vector;	//储存公民权政策
vector<religious_policy> religious_policy_vector;	//储存宗教政策
vector<trade_policy> trade_policy_vector;	//储存贸易政策
vector<war_policy> war_policy_vector;	//储存战争政策
...
{
	string str;
	while ( issues_input >> str )
	{
		int brace = 0;	//进入{的等级
		if ( str [ 0 ] == '#' )
		{
			char eat [ 1000 ];
			issues_input.getline ( eat , 1000 );
			continue;
		}
		if ( str == "trade_policy" )
		{
			string trade_policy_name;
			while ( issues_input >> trade_policy_name )
			{
				if ( trade_policy_name [ 0 ] == '}' )
				{
					break;
				}
				double max_tariff , min_tariff;
				string sym;
				issues_input >> sym >> sym >> sym >> sym >> max_tariff >> sym >> sym >> min_tariff >> sym;
				trade_policy_vector.push_back ( trade_policy ( trade_policy_name , max_tariff , min_tariff ) );
			}
		}
		if ( str == "economic_policy" )
		{
